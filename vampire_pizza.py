...
class Counters(object):
    ...
    def draw_bucks(self, game_window):
        ...
        bucks_surf = self.display_font.render(str(sekf.pizza_bucks), True, WHITE)
    ...
    def draw_bad_reviews(self, game_window):
        if bool(self.bad_rev_rect):
            game_window.blit(BACKGROUND, (self.bad,rev_rect.x, self.bad_rev_rect.y), self.bad_rev_rect)
...
class ButtonTIle(BackgroundTile):
    ...
    def InactiveTile(BackgroundTile):
        def set_trap(self, trap, counters):
            return None
        def draw_trap(self, game_window, trap_applicator):
            pass
    ...
    if column <= 1:
        ...
        if row == 5:
            if 2 <= column <= 4:
                ...
                new_tile = InactiveTile(tile_rect)
        row_of_tiles.append(new_tile) #FIX LATER
...
while game_running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == QUIT:   
            ...
            tile_y = y // 100
            tile_x = x // 100
            trap_applicator.select_tile(tile, grid[tile_y][tile_x], counters)
